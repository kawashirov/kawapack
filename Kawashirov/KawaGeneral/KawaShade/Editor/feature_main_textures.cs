...
namespace Kawashirov.KawaShade {
	...
	internal static partial class KawaShadeCommons {
		...
		internal static readonly string F_Cutout_Classic = "KawaShade_Feature_Cutout_Classic";
		internal static readonly string F_Cutout_RangeRandom = "KawaShade_Feature_Cutout_RangeRandom";
	...
	public partial class KawaShadeGenerator {
		...
		private void ConfigureFeatureMainTex(ShaderSetup shader) {
			...
			shader.TagEnum(KawaShadeCommons.F_MainTex, mainTex);
		...
		private void ConfigureFeatureCutoff(ShaderSetup shader) {
			...
			if (forward_on) {
				...
				shader.TagEnum(KawaShadeCommons.F_Cutout_Forward, forward_mode);
			}
			if (shadow_on) {
				...
				shader.TagEnum(KawaShadeCommons.F_Cutout_ShadowCaster, shadow_mode);
			}

			shader.TagBool(KawaShadeCommons.F_Cutout_Classic, prop_classic);
			if (prop_classic) {
				shader.properties.Add(new PropertyFloat() { name = "_Cutoff", defualt = 0.5f, range = new Vector2(0, 1) });
			}
			shader.TagBool(KawaShadeCommons.F_Cutout_RangeRandom, prop_range);
			if (prop_range) {
		...
		private void ConfigureFeatureEmission(ShaderSetup shader) {
			shader.TagBool(KawaShadeCommons.F_Emission, emission);
			if (emission) {
				shader.forward.defines.Add("EMISSION_ON 1");
				shader.TagEnum(KawaShadeCommons.F_EmissionMode, emissionMode);
				switch (emissionMode) {
		...
		private void ConfigureFeatureNormalMap(ShaderSetup shader) {
			shader.TagBool(KawaShadeCommons.F_NormalMap, bumpMap);
			if (bumpMap) {
	...
	public partial class KawaShadeGeneratorEditor {
		private void TexturesGUI() {
			...
			using (new EditorGUI.IndentLevelScope()) {
				...
				KawaGUIUtility.PropertyEnumPopupCustomLabels(mainTex, "Main (Albedo) Texture", KawaShadeCommons.mainTexKeywordsNames);
				...
				KawaGUIUtility.PropertyEnumPopupCustomLabels(cutout, "Cutout Mode", KawaShadeCommons.cutoutModeNames);
				...
				using (new EditorGUI.DisabledScope(!emission.boolValue)) {
					using (new EditorGUI.IndentLevelScope()) {
						...
						KawaGUIUtility.PropertyEnumPopupCustomLabels(emissionMode, "Mode", KawaShadeCommons.emissionMode);
					}
				}
		...
		protected void OnGUI_Textures() {
			...
			using (new EditorGUI.IndentLevelScope()) {
				...
				using (new EditorGUI.IndentLevelScope()) {
					...
					LabelEnumDisabledFromTagMixed<CutoutMode>("Forward Pass Cutout Mode", KawaShadeCommons.F_Cutout_Forward);
					LabelEnumDisabledFromTagMixed<CutoutMode>("Shadow Caster Cutout Mode", KawaShadeCommons.F_Cutout_ShadowCaster);
				...
				using (new EditorGUI.DisabledScope(!f_emission)) {
					...
					if (f_emission) {
						using (new EditorGUI.IndentLevelScope()) {
							LabelEnumDisabledFromTagMixed("Emission Mode", KawaShadeCommons.F_EmissionMode, KawaShadeCommons.emissionMode);
