...
inline uint rnd_apply_time(uint rnd) {
	...
	// sin и cos времени в uint изменяются только в низких значения
	// (по всей видимости, только младшие биты мантиссы активно меняются)
	// Чистое время же в долгосрочной перспективе имеет повторяющиеся патрены
	// Использование и чистого времени, и синусов-косинусов позволяет справиться с этой проблемой.
	rnd = rnd * (asuint(_Time.y) + asuint(_SinTime.w)) + (asuint(_Time.y) + asuint(_CosTime.w));
	return rnd;
}
