...
half4 frag_forwardbase(FRAGMENT_IN i) : COLOR {
	...
	fps_apply_uv(uv);
	// apply_bitloss2(uv, uint2(36266, 51561), uint2(43336, 61837), true);
	...
	apply_bitloss3(emissive, uint3(48442, 56430, 41145), uint3(41357, 54593, 33604), true);
	...
	glitter_apply_color(i, uv, normal3, wsvd_norm, albedo.rgb, emissive);
	
	// Заменяюще-затеняющие эффекты
	fps_apply_colors(albedo.rgb, emissive);
	wnoise_apply(i, albedo.rgb, emissive);
	...
	apply_bitloss3(emissive, uint3(46086, 55087, 46506), uint3(50455, 57529, 40378), true);
	...
	apply_bitloss3(glossy, uint3(34900, 57624, 57978), uint3(53045, 47927, 54312), true);
